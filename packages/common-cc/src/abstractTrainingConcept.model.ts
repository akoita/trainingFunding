...
const TrainingAppLifecycleStatusYupSchema = () =>
  yup
    .string()
    .oneOf(
      Object.keys(TrainingAppLifecycleStatus).map(
        k => TrainingAppLifecycleStatus[k]
      )
    );

export abstract class AbstractTrainingConceptModel<
  T extends AbstractTrainingConceptModel<any>
> extends ConvectorModel<T> {
  @ReadOnly()
  @Required()
  @Validate(yup.string())
  public id: string;
  ...
  @Required()
  @Validate(TrainingAppLifecycleStatusYupSchema())
  public status: TrainingAppLifecycleStatus;


  public withStatus(
    @Param(TrainingAppLifecycleStatusYupSchema())
  ...
  public static async getOne<T extends ConvectorModel<any>>(
    ...
    if (content && model && content.type !== modelType) {
      throw new Error(
        `Possible ID collision, element with id ${id} type's is ${
          content.type
        } and not ${modelType}`
      );
    }
