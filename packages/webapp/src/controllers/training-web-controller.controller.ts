...
export class TrainingWebControllerController {
    ...
    public async getTrainingById(@param.query.string('id')trainingId: string): Promise<Training> {
    ...
    public async submitTrainingApplication(@param.query.string('id')trainingId: string) {
    ...
    public async getTrainingsByCandidatesIds(@param.array('ids', 'query', {type: 'string'})  candidatesIds: string[]): Promise<Training[]> {
        return await TrainingControllerBackEnd.getTrainingsByCandidatesIds(candidatesIds);
    }


    @get('trainings/byProcessStatus', {
        description: 'Returns the list of the trainings following the given process status',
        responses: {
            200: {
                description: 'the list of the trainings following the given process status',
                content: {
                    'application/json': {
                        schema: {
                            type: 'array',
    ...
    public async getTrainingsByProcessStatus(@param.array('status', 'query', {type: 'string'})  trainingProcessStatus: TrainingProcessStatus[]): Promise<Training[]> {
        return await TrainingControllerBackEnd.getTrainingsByProcessStatus(trainingProcessStatus);
    }
