...
describe('Candidate', function () {
    ...
    it('should find the candidate matching given name', function () { return tslib_1.__awaiter(_this, void 0, void 0, function () {
        var aboubakar, soumaya, dede, candidates, candidates2;
        return tslib_1.__generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    aboubakar = new src_1.Candidate();
                    aboubakar.id = uuid();
                    aboubakar.created = Date.now();
                    aboubakar.modified = Date.now();
                    aboubakar.firstName = 'Aboubakar';
                    aboubakar.lastName = 'Koïta';
                    aboubakar.status = common_cc_1.TrainingAppLifecycleStatus.Open;
                    ...
                    return [4, candidateCtrl.createCandidate(aboubakar)];
                case 1:
                    _a.sent();
                ...
                case 3:
                    _a.sent();
                    return [4, candidateCtrl.searchCandidate("Koïta")];
                case 4:
                    candidates = _a.sent();
                    if (Array.isArray(candidates)) {
                        chai_1.expect(candidates.map(function (c) { return new src_1.Candidate(c); })).to.have.same.deep.members([aboubakar, soumaya, dede]);
                    }
                    else {
                ...
                case 5:
                    ...
                    if (Array.isArray(candidates2)) {
                        chai_1.expect(candidates2.map(function (c) { return new src_1.Candidate(c); })).to.have.same.deep.members([aboubakar]);
                    }
                    else {
                        throw new Error('no candidate matching name "Koïta"');
                    }
